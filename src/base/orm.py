...
def exists_model_from_data(model: Union[Model, Any], *args, raise_exception: bool = False, raise_error_text: Optional[str] = None, **kwargs) -> bool:
    """Если raise_exception=True то выдаст ошибку, это похож как serializer.is_valid(raise_exception)."""
    if not issubclass(model, models.Model):
        message = "%s is not a subclass models.Model" % model.__class__.__name__
